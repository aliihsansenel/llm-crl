...
import supabase, { addVocabToPrivateList } from "../lib/supabase";
...
export default function DiscoverPage() {
  const [active, setActive] = useState<"vocabs" | "lists">("vocabs");
  ...
  return (
    <div className="p-6">
      ...
      <Tabs
        value={active}
        onValueChange={(v: any) =>
          setActive(v === "lists" ? "lists" : "vocabs")
        }
      >
        <TabsList>
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (error) throw error;
        if (!mounted) return;
        setMeanings((data || []).map((r: any) => r.itself));
      } catch (err: any) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!mounted) return;
        setItems((vocabsRes || []).map((r: any) => r.itself));
      } catch (err: any) {
